...
class BertXAttentionOld(nn.Module):
    def __init__(self, config, ctx_dim=None):
        super().__init__()
...
class BertOutAttentionOld(nn.Module):
    def __init__(self, config, ctx_dim=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0:
            raise ValueError(
...
class VisionEncoder(nn.Module):
    ...
    def forward(self, visn_input, visn_token):
...
class VisionEncoder_org(nn.Module):
    def __init__(self, vision_size, config):
        super().__init__()
        feat_dim = vision_size
        #pos_dim = VISUAL_CONFIG.visual_pos_dim
...
class DicModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, position_ids=None, head_mask=None, img_feats=None, img_mask = None):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
